...
bioJs.controller('DashboardCtrl',function($scope,$http){
  ...
  $http({method: 'GET', url: 'http://localhost/onco/app/webservice.json'}).success(function(data){
    ...
    for(var i=0; i< data.treatmentList.length; i++){
      ...
      $scope.startMonth = new Date(data.treatmentList[i].startDate.substring(0,4)+'-1-01').getMonth()+1
      $scope.startYear = data.treatmentList[i].startDate.substring(8,12);
      $scope.endDay = data.treatmentList[i].endDate.substring(4,6);
      $scope.endMonth = new Date(data.treatmentList[i].endDate.substring(0,4)+'-1-01').getMonth()+1
      $scope.endYear = data.treatmentList[i].endDate.substring(8,12);

    events.push({dates: [new Date($scope.startYear, $scope.startMonth, $scope.startDay), 
  new Date($scope.endYear, $scope.endMonth, $scope.endDay)], title: data.treatmentList[i].treatmentDescription, section: i});
    sections.push({dates: [new Date($scope.startYear, $scope.startMonth, $scope.startDay), 
  new Date($scope.endYear, $scope.endMonth, $scope.endDay)], title:data.treatmentList[i].treatmentName, section: i, attrs: {fill: "#e3f0fe"}}
);
    }
    $scope.startDay = data.treatmentList[0].startDate.substring(4,6);
    $scope.startMonth = new Date(data.treatmentList[0].startDate.substring(0,4)+'-1-01').getMonth()+1
    $scope.startYear = data.treatmentList[0].startDate.substring(8,12);
